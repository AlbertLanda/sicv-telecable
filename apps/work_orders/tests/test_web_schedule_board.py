"""Pruebas del tablero semanal de programación de órdenes de trabajo."""
...
class ScheduleBoardTestCase(WorkOrderTestCase):
    """Base común: un usuario de consulta y otro con permiso de programación."""

    DEFAULT_REASON = "Ajuste de programación para prueba"
    ...
    def setUp(self):
        ...
        schedule_permission = Permission.objects.get(
            codename="schedule_workorder",
            content_type__app_label="work_orders",
        )
        ...
        self.dispatcher = self.make_user("programacion1")
        self.dispatcher.user_permissions.add(view_permission, schedule_permission)

    def make_user(self, username):
        from apps.accounts.models import User

        # SALES no hereda la matriz base de ATC. Así estas pruebas aíslan los
        # permisos explícitos de ver/programar sin contradecir que ATC sí debe
        # poder programar por definición de su rol operativo.
    ...
    def every_order(self, response):
        columns = [response.context["unscheduled_column"]] + list(
            response.context["day_columns"]
        )
    ...
    def reschedule(self, order, day, reason=None):
...
class ScheduleBoardAccessTests(ScheduleBoardTestCase):
    ...
    def test_with_view_permission_it_renders(self):
        ...
        self.assertTemplateUsed(response, "work_orders/work_order_schedule_board.html")
...
class ScheduleBoardWeekTests(ScheduleBoardTestCase):
    ...
    def test_any_day_of_a_week_opens_that_week(self):
        ...
        self.assertEqual(
            response.context["week_start"], self.week_start + timedelta(weeks=2)
        )
    ...
    def test_navigating_to_that_week_shows_them(self):
        ...
        self.login(self.viewer)
        self.assertIn(order, self.every_order(self.board(fecha=target.isoformat())))
...
class ScheduleBoardColumnTests(ScheduleBoardTestCase):
    def test_an_order_lands_in_the_column_of_its_scheduled_day(self):
        ...
        self.login(self.viewer)
        column = next(
            c for c in self.board().context["day_columns"] if c["date"] == self.today
        )
...
class RescheduleEndpointTests(ScheduleBoardTestCase):
    ...
    def test_a_missing_date_is_reported(self):
        ...
        response = self.client.post(
            self.reschedule_url(order),
            data=json.dumps({"reason": self.DEFAULT_REASON}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 400)
