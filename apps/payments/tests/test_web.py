...
class SidebarEntryTests(AccountScreensTestCase):
    """Las entradas del menú: llevan al abonado elegido o a buscarlo."""

    def test_with_a_selected_customer_it_opens_that_account(self):
        self.login(self.viewer)
    ...
    def test_without_a_selected_customer_it_sends_to_the_search(self):
        """Sin abonado elegido no se abre una cuenta vacía.

        Una pantalla sin cargos parecería afirmar que el abonado no debe nada,
        cuando lo que pasa es que todavía no se eligió a ninguno.
        """
        self.login(self.viewer)
    ...
    def test_a_stale_selection_does_not_break_the_menu(self):
        """El cliente elegido pudo dejar de existir desde la última sesión."""
        self.login(self.viewer)
        ...
        response = self.client.get(reverse("payments:selected_history"))

        self.assertRedirects(response, reverse("customers:search"))

    def test_the_menu_shows_the_three_entries_to_who_may_see_them(self):
        self.login(self.viewer)

        response = self.client.get(self.debt_url())

        self.assertContains(response, reverse("payments:selected_debt"))
        self.assertContains(response, reverse("payments:selected_history"))
        self.assertContains(response, reverse("payments:selected_receipts"))

    def test_the_menu_hides_what_the_user_may_not_see(self):
        user = self.make_user("solodeuda", permissions=["view_charge"])
        self.login(user)

        response = self.client.get(self.debt_url())

        self.assertContains(response, reverse("payments:selected_debt"))
        self.assertNotContains(response, reverse("payments:selected_history"))
        self.assertNotContains(response, reverse("payments:selected_receipts"))
